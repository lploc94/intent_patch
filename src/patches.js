...
function buildPatches(files, pcSymbols, msSymbols, mpSymbols, extractedDir) {
  ...
  if (files.agent_missing_ipc) {
    ...
    patches.push({
      name: 'Patch 10B: context-rich enhancer via CLI (providerTools)',
      file_key: 'agent_missing_ipc',
      patch_type: 'text_replace',
      search: (
...
function _buildPatch10BReplace() {
  return (
    "// intent-patch: context-rich enhancer via CLI (providerTools)\n"
    ...
    + "            let enhancedPrompt;\n"
    + "            if (_tool) {\n"
    + "                const _cliArgs = Array.isArray(_entry.args) ? _entry.args : [];\n"
...
// Patch 10B v2 replace text (via CLI with _cfg.tool) — used as search_alt for v2→v3 upgrade path
function _buildPatch10BV2Replace() {
  return (
...
// Patch 10B v3 replace text (providerTools + augmentCLI fallback) — used as search_alt for v3→v4 upgrade path
// Uses old system prompt (the one deployed with providerTools in commit 0420124)
function _buildPatch10BV3Replace() {
  return (
    "// intent-patch: context-rich enhancer via CLI (providerTools)\n"
    + "            const _cfgPath = _path.join(_os.homedir(), '.intent-patch', 'enhancer.json');\n"
    + "            let _cfg = {};\n"
...
// Patch 10B v3b replace text (providerTools + Vietnamese sysPrompt + augmentCLI fallback) — v3→v4 upgrade
// For users who ran the patcher between commits cecf3fb (Vietnamese sysPrompt) and 4cc5e9d (throw-only)
function _buildPatch10BV3bReplace() {
...
// Patch 10B v1 replace text (augmentCLI-only) — used as search_alt in v2 (now dead code, kept for reference)
